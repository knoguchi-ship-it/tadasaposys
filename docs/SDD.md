# システム詳細設計書 (SDD) — タダサポ管理システム v1.12.12

**Version:** 1.12.12
**Date:** 2026/06/16
**Status:** Released

> **v1.12.12 追補（サポート終了メールを「送信前に編集」する方式へ・フロントのみ）**
> - 完了報告時のサポート終了メールを、自動送信（v1.12.11）から **完了報告後にメール作成モーダルを開き、ログイン中メンバーが件名・本文をその場で編集して送信**する方式へ変更。既存スレッドへ返信／無ければ新規、×で閉じれば送信なし、下書き保存可。
> - メールモーダルに新モード `closing` を追加。送信は既存 `sendCaseEmail`/`sendNewCaseEmail` を再利用し、`handleSendEmail` の reply-or-new 分岐に合流（DRY）。テンプレ初期値 `MAIL_CLOSING_SUBJECT`/`MAIL_CLOSING_BODY` は管理設定で編集可（v1.12.11 で追加済み）。**バックエンド・データモデルの変更なし**。
> - テスト: `tests/e2e/11-closing-email.spec.ts` を新フローへ更新。E2E 65件・単体92件、全PASS。

> **v1.12.11 追補（サポート終了メール機能の追加）**
> - 完了報告モーダルに「サポート終了メールを送信する」チェック（既定ON）を追加。完了と同時に相談者へ終了連絡メールを**既存スレッドへ返信**送信（無ければ新規）。送信は既存 `sendCaseEmail`/`sendNewCaseEmail` を再利用し、サブ担当を自動CC。完了報告以外の**ステータス追加変更なし**。送信失敗は非致死（完了報告は保存済み）。
> - 件名・本文は設定キー `MAIL_CLOSING_SUBJECT` / `MAIL_CLOSING_BODY` で管理（設定管理ダイアログ「メール」タブ「サポート終了メール」）。タグ `{{名前}} {{事業所名}} {{担当者名}}` を置換。`getMasters().emailTemplates` に `closingSubject`/`closingBody` を追加。シート列・IDX 定数の変更はなし（設定シートへの Key-Value 追記のみ）。
> - テスト: `tests/e2e/11-closing-email.spec.ts`（2件）。E2E 63→65件・単体92件、全PASS。

> **v1.12.10 追補（メール下書きの再編集・送信導線を追加・フロントのみ／設計影響軽微）**
> - **不具合修正**: 保存済みメール下書き（特に既存スレッドへの返信下書き）に案件詳細からの一覧表示・再編集・送信導線が無く、同一操作経路でモーダルを開き直さない限り復元できなかった。案件詳細に「下書き一覧」セクションを新設し、`listDraftsForCase` の結果を種別・件名・本文プレビュー・更新日時付きで一覧化。「開く」で該当メールモーダル（mode/threadId/CC/BCC/件名/本文/ツール）を復元して再編集・送信、「削除」で破棄できる。未対応案件でも下書きを読み込む。
> - **バックエンド変更なし**: 既存の `saveDraft`/`loadDraft`/`deleteDraft`/`listDraftsForCase`（メール下書きシート・11列）をそのまま利用。新規 API・シート列・IDX の変更なし。アクセシビリティ改善としてメールモーダルの閉じるボタンに `aria-label="閉じる"` を付与。
> - **テスト**: `tests/e2e/10-draft-list.spec.ts`（2件）を追加。E2E 61→63 件・単体 92 件、全 PASS・回帰なし。
