...
> **最終更新: 2026/06/16 / v1.12.10 を本番 @156 にデプロイ済み・GitHub master に push 済み（E2E 63件・単体 92件 全PASS）。**
...
### 直近セッションで完了したこと（v1.12.10）
- **メール下書きの再編集・送信導線を追加（不具合修正）**: 保存済みメール下書き（特に**既存スレッドへの返信**下書き）に案件詳細からの一覧・再編集・送信導線が無く、同一操作経路でモーダルを開き直さない限り復元できなかった問題を修正。
  - 案件詳細のメールスレッド上部に **「下書き一覧」** を新設（`index.html`）。種別・件名・本文プレビュー・更新日時を表示し、**「開く」で該当メールモーダルを復元（mode/threadId/CC/BCC/件名/本文/ツール）して再編集・送信**、**「削除」で破棄**。未対応案件でも下書きを読込。保存・送信・削除と「下書きあり」バッジを相互同期。「開く」時は自動読込プロンプトを抑止。
  - **バックエンド変更なし**（既存 `listDraftsForCase`/`loadDraft`/`deleteDraft` を利用）。メールモーダルの閉じるボタンに `aria-label="閉じる"` を付与（a11y改善）。
  - **E2E 追加**: `tests/e2e/10-draft-list.spec.ts`（2件）。「壊れたコードで失敗・正しいコードで合格」を確認。E2E 61→63件・単体92件 全PASS・回帰なし。

### 🔴 まずやること（運用・確認系。コードは触らず数分で完了）
1. **GASデプロイ管理画面で @156 の Webapp 設定を目視確認**（実行＝アクセスしているユーザー／アクセス＝NPO法人タダカヨ内の全員）。
2. **本番URLで v1.12.10 を確認**：案件詳細でメール下書きを保存→「下書き (◯件)」が表示され、「開く」で再編集・送信、「削除」で破棄できればOK。
...
### 読む順番
1. 本節（現状把握）→ 2. §11（残タスク詳細）→ 3. §9（既知の制約）→ 4. §12（v1.12.10 までの版数履歴）→ 5. `CLAUDE.md`/`AGENTS.md`（開発ルール）
...
| **Playwright E2E テストスイート（63テスト）** | ✅ | **v1.11.5〜（T1/T2/R3 で +10、下書き一覧で +2）** |
...
| SDD.md / Manual.md ドキュメント | v1.12.10 対応済み | 2026/06/16 更新 |
