...
**Project:** タダサポ管理システム
**Version:** 1.12.12（現行リリース）
**Date:** 2026/06/16
**Author:** Development Team

---

## 📌 次の担当者へ（最初にこの節だけ読めば現状が掴めます）

> **最終更新: 2026/06/16 / v1.12.12 を本番 @158 にデプロイ済み・GitHub master に push 済み（E2E 65件・単体 92件 全PASS）。**
> 本番・master・GitHub の3者は一致しています。安心して続きから着手できます。固定 deploymentId / URL 不変（新規デプロイなし）。
> ⚠️ デプロイ直後の運用確認として、GAS デプロイ管理画面で Webapp 設定（実行＝アクセスしているユーザー／アクセス＝NPO法人タダカヨ内の全員）をブラウザで目視確認すること（CLIでは確認不可）。

### 直近セッションで完了したこと（v1.12.12）
- **サポート終了メールを「送信前に編集」する方式へ変更**: v1.12.11 はテンプレ自動送信だったが、**完了報告のあとメール作成モーダルを開き、ログイン中メンバーが件名・本文をその場で書き換えて送信**する方式にした（既存スレッドへ返信／無ければ新規・×で閉じれば送信なし・下書き保存可）。案件ごと・メンバーごとに自由に文面調整できる。
  - メールモーダルに新モード `closing` を追加（見出し「サポート終了メール送信」）。送信は既存 `sendCaseEmail`/`sendNewCaseEmail` を再利用し `handleSendEmail` の reply-or-new 分岐に合流（DRY）。テンプレ初期値 `MAIL_CLOSING_SUBJECT`/`MAIL_CLOSING_BODY` は管理設定で編集可（v1.12.11 で追加済み）。**バックエンド変更なし**（`コード.js` はヘッダ版数のみ）。
  - **E2E 更新**: `tests/e2e/11-closing-email.spec.ts` を新フローへ（ON で終了メール作成モーダルが開き編集→送信／OFF で開かない）。「モーダル非表示の壊れたコードで失敗・正しいコードで合格」を確認。E2E 65件・単体92件 全PASS・回帰なし。

### 🔴 まずやること（運用・確認系。コードは触らず数分で完了）
1. **GASデプロイ管理画面で @158 の Webapp 設定を目視確認**（実行＝アクセスしているユーザー／アクセス＝NPO法人タダカヨ内の全員）。
2. **本番URLで v1.12.12 を確認**：完了報告で「サポート終了メールを送信する」ON→完了後にメール作成画面が開き、本文を編集して送信できればOK（OFF なら開かない）。
3. **（前リリース残）本番URLで R3 を確認**：日程確定→方法「対面」等＋「カレンダーに登録」ON で「**この方法ではカレンダーへの予定登録は行われません（対応記録のみ保存されます）**」と出ればOK。
4. **R4（任意・無害）**：GASエディタで `temp/index`・`temp/コード` を手動削除（手順 RUNBOOK §2-4）。

### 🟡 あなたが判断・着手する開発タスク（詳細は §11）
| 優先 | ID | 内容 |
|---|---|---|
| **要判断** | **R3** | 非Meet/Zoom方法のカレンダー登録を **A:実装する／B:登録UIを出さない／C:現状維持で仕様確定** から決める。判断材料はバックエンド `updateSupportRecord`（現状 Zoom/GoogleMeet のみイベント作成） |
| 中 | 7-1 | CSV/スプレッドシートエクスポート |
| 低 | 7-3 | 検索のスマート化（条件保存） |
| 任意 | R2残 | サーバ権限ガードの自動テスト整備（現harnessはGAS Session依存で未対応＝モック設計が必要） |

### 読む順番
1. 本節（現状把握）→ 2. §11（残タスク詳細）→ 3. §9（既知の制約）→ 4. §12（v1.12.12 までの版数履歴）→ 5. `CLAUDE.md`/`AGENTS.md`（開発ルール）
...
- **本番デプロイバージョン**: @158（v1.12.12、2026/06/16 デプロイ。固定 deploymentId / URL 不変。サポート終了メールを送信前に編集する方式へ＝フロントのみ）。前版 @157（v1.12.11）はサポート終了メール機能の追加、@156（v1.12.10）はメール下書き一覧の追加、@155（v1.12.9）は R3 虚偽UI文言の修正、@154（v1.12.8）で S1 Stage4＝Read切替トグル `CASE_KEY_READ_VIA_MAP` 公開・Backfill 実行済（既存136案件に case_id 付与・診断クリア）
...
| **メール下書き一覧（案件詳細から再編集・送信・削除）** | ✅ | **v1.12.10** |
| **サポート終了メール（完了報告時に送信・送信前にメンバーが文面編集可・本文初期値は設定で編集可）** | ✅ | **v1.12.11〜v1.12.12** |
| **Playwright E2E テストスイート（65テスト）** | ✅ | **v1.11.5〜（T1/T2/R3 +10、下書き一覧 +2、終了メール +2）** |
...
| SDD.md / Manual.md ドキュメント | v1.12.12 対応済み | 2026/06/16 更新 |
...
| **v1.12.10** | 2026/06/16 | メール下書きの再編集・送信導線を追加（不具合修正）: 保存済み下書き（特に既存スレッドへの返信）が案件詳細から一覧・再編集・送信できなかった問題を、案件詳細の「下書き一覧」新設で解消（`index.html`）。種別/件名/本文プレビュー/更新日時を表示し「開く」でメールモーダル復元→再編集・送信、「削除」で破棄。未対応案件でも読込。保存/送信/削除と「下書きあり」バッジを同期。メールモーダル閉じるボタンに `aria-label` 付与。バックエンド変更なし（既存 `listDraftsForCase` 等を利用）。E2E `10-draft-list.spec.ts`（2件）追加・E2E 61→63件・単体92件 全PASS。**@156 デプロイ済み（固定 deploymentId / URL 不変）** |
| **v1.12.11** | 2026/06/16 | サポート終了メールの送信機能を追加: 完了報告モーダルに「サポート終了メールを送信する」チェック（既定ON）を新設し、完了と同時に相談者へ終了連絡メールを既存スレッドへ返信送信（無ければ新規・サブ担当を自動CC・ステータス追加変更なし・送信失敗は非致死）。件名/本文は設定キー `MAIL_CLOSING_SUBJECT`/`MAIL_CLOSING_BODY` で編集可能（設定管理「メール」タブ）。送信は既存 `sendCaseEmail`/`sendNewCaseEmail` を再利用（DRY）。E2E `11-closing-email.spec.ts`（2件）追加・E2E 63→65件・単体92件 全PASS。**@157 デプロイ済み（固定 deploymentId / URL 不変）** |
| **v1.12.12** | 2026/06/16 | サポート終了メールを「送信前に編集」する方式へ変更: v1.12.11 のテンプレ自動送信を、完了報告後にメール作成モーダルを開きログイン中メンバーが件名・本文をその場で書き換えて送信する方式に（既存スレッドへ返信／無ければ新規・×で閉じれば送信なし・下書き保存可）。メールモーダルに新モード `closing` を追加し送信は既存 `sendCaseEmail`/`sendNewCaseEmail` を再利用（`handleSendEmail` reply-or-new 分岐に合流・DRY）。テンプレ初期値は管理設定で編集可（v1.12.11）。バックエンド変更なし（フロントのみ）。E2E `11-closing-email.spec.ts` を新フローへ更新・E2E 65件・単体92件 全PASS。**@158 デプロイ済み（固定 deploymentId / URL 不変）** |
