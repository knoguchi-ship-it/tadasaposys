...
> **最終更新: 2026/06/11 / v1.12.9 を本番 @155 にデプロイ済み・GitHub master に push 済み。**
> 本番・master・GitHub の3者は一致しています。安心して続きから着手できます。

### 直近セッションで完了したこと（v1.12.9）
- **T1/T2**: E2E テストを追加（日程確定モーダル＝`tests/e2e/08-scheduling.spec.ts` / 管理ステータス遷移＝`09-admin-status-transition.spec.ts`）。**E2E 51→61件・単体92件 全PASS**。
- **T3**: `コード.js` 最上位の不変定数を `const`／可変キャッシュを `let` 化（無挙動）。
- **R2**: `updateSupportHistory` の担当者チェックは **元から実装済み**（`ensureCaseEditableByActor_`）と判明 → ドキュメントの誤記を訂正（実装作業なし）。
- **R1**: 不整合修復スクリプトは **S1 根治で動機消失** → 縮退（クローズ候補）。
- **R3（一部のみ）**: 日程確定で非Meet/Zoom方法（対面/電話等/メール等）に出ていた**虚偽の「カレンダーに登録します」表示を修正**。**カレンダー登録の本体実装は未着手＝あなたの判断事項**（下記）。
- **R4**: GAS残存スタブ削除手順を RUNBOOK §2-4 に明記（ローカル衛生は完了済み）。

### 🔴 まずやること（運用・確認系。コードは触らず数分で完了）
1. **GASデプロイ管理画面で @155 の Webapp 設定を目視確認**（実行＝アクセスしているユーザー／アクセス＝NPO法人タダカヨ内の全員）。
2. **本番URLで R3 を確認**：日程確定→方法「対面」等＋「カレンダーに登録」ON で「**この方法ではカレンダーへの予定登録は行われません（対応記録のみ保存されます）**」と出ればOK。
...
### 読む順番
1. 本節（現状把握）→ 2. §11（残タスク詳細）→ 3. §9（既知の制約）→ 4. §12（v1.12.9 までの版数履歴）→ 5. `CLAUDE.md`/`AGENTS.md`（開発ルール）
