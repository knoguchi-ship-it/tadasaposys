...
> **最終更新: 2026/06/16 / v1.12.10 をローカルで実装・テスト完了（E2E 63件・単体 92件 全PASS）。**
> ⚠️ **本番は @155（v1.12.9）のまま。v1.12.10 は git commit / GitHub push / clasp デプロイをまだ実施していません**（フロントのみの変更）。本番反映が必要な場合は §10/§4 のデプロイ手順を実施してください。
...
### 🔴 まずやること（運用・確認系）
1. **v1.12.10 を本番反映するか判断**。反映する場合は §4/§10 の手順で `git commit` → `clasp push --force` → `clasp deploy -i …`（固定 deploymentId・`-i` 付き）→ GAS デプロイ管理画面で Webapp 設定（実行＝アクセスしているユーザー／アクセス＝NPO法人タダカヨ内の全員）を目視確認。
2. **（前リリース残）本番URLで R3 を確認**：日程確定→方法「対面」等＋「カレンダーに登録」ON で「**この方法ではカレンダーへの予定登録は行われません（対応記録のみ保存されます）**」と出ればOK。
3. **R4（任意・無害）**：GASエディタで `temp/index`・`temp/コード` を手動削除（手順 RUNBOOK §2-4）。
...
| **v1.12.10** | 2026/06/16 | メール下書きの再編集・送信導線を追加（不具合修正）: 保存済み下書き（特に既存スレッドへの返信）が案件詳細から一覧・再編集・送信できなかった問題を、案件詳細の「下書き一覧」新設で解消（`index.html`）。種別/件名/本文プレビュー/更新日時を表示し「開く」でメールモーダル復元→再編集・送信、「削除」で破棄。未対応案件でも読込。保存/送信/削除と「下書きあり」バッジを同期。メールモーダル閉じるボタンに `aria-label` 付与。バックエンド変更なし（既存 `listDraftsForCase` 等を利用）。E2E `10-draft-list.spec.ts`（2件）追加・E2E 61→63件・単体92件 全PASS。**未デプロイ（本番は @155 / v1.12.9 のまま）** |
