...
> **最終更新: 2026/06/16 / v1.12.11 を本番 @157 にデプロイ済み・GitHub master に push 済み（E2E 65件・単体 92件 全PASS）。**
...
### 直近セッションで完了したこと（v1.12.11）
- **サポート終了メールの送信機能を追加**: 完了報告モーダルに「サポート終了メールを送信する」チェック（**既定ON**）を新設し、完了と同時に相談者へ終了連絡メールを**既存スレッドへ返信**送信（無ければ新規）。サブ担当を自動CC。完了報告以外の**ステータス追加変更なし**。送信失敗は非致死（完了報告は保存済み・エラートースト表示）。
  - 件名・本文は**管理設定で編集可能**（`MAIL_CLOSING_SUBJECT`/`MAIL_CLOSING_BODY`、設定管理「メール」タブ「サポート終了メール」）。タグ `{{名前}} {{事業所名}} {{担当者名}}` 置換。
  - 送信は既存 `sendCaseEmail`/`sendNewCaseEmail` を再利用（DRY）。スキーマ migration・`getEditableSettingsKeys_`/`SETTINGS_LABEL_MAP_`/`getMasters().emailTemplates`、フロント `MOCK_MASTERS`/`MOCK_SETTINGS`/`settingsMeta` を同期。
  - **E2E 追加**: `tests/e2e/11-closing-email.spec.ts`（2件）。「送信無効化の壊れたコードで失敗・正しいコードで合格」を確認。E2E 63→65件・単体92件 全PASS・回帰なし。

### 🔴 まずやること（運用・確認系。コードは触らず数分で完了）
1. **GASデプロイ管理画面で @157 の Webapp 設定を目視確認**（実行＝アクセスしているユーザー／アクセス＝NPO法人タダカヨ内の全員）。
2. **本番URLで v1.12.11 を確認**：完了報告画面に「サポート終了メールを送信する」チェックがあり、完了時に終了メール送信トーストが出る／管理設定「メール」タブに「サポート終了メール」件名・本文が表示・編集できればOK。
...
### 読む順番
1. 本節（現状把握）→ 2. §11（残タスク詳細）→ 3. §9（既知の制約）→ 4. §12（v1.12.11 までの版数履歴）→ 5. `CLAUDE.md`/`AGENTS.md`（開発ルール）
...
| **サポート終了メール（完了報告時に送信・本文は設定で編集可）** | ✅ | **v1.12.11** |
...
| SDD.md / Manual.md ドキュメント | v1.12.11 対応済み | 2026/06/16 更新 |
