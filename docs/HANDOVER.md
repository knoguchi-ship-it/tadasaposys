...
> **最終更新: 2026/06/16 / v1.12.11 をローカルで実装・テスト完了（E2E 65件・単体 92件 全PASS）。**
> ⚠️ **本番は @156（v1.12.10）のまま。v1.12.11 は git commit / GitHub push / clasp デプロイをまだ実施していません**（今回はバックエンドにも設定キー追加あり＝`コード.js` の実差分あり）。本番反映は §10/§4 のデプロイ手順を実施してください。
...
### 🔴 まずやること（運用・確認系）
1. **v1.12.11 を本番反映するか判断**。反映する場合は §4/§10 の手順で `git commit` → `clasp push --force` → `clasp deploy -i …`（固定 deploymentId・`-i` 付き）→ GAS デプロイ管理画面で Webapp 設定（実行＝アクセスしているユーザー／アクセス＝NPO法人タダカヨ内の全員）を目視確認。
...
- **本番デプロイバージョン**: @156（v1.12.10、2026/06/16 デプロイ。固定 deploymentId / URL 不変。メール下書き一覧の追加＝フロントのみの変更。`コード.js` はヘッダ版数のみ変更で挙動不変）。前版 @155（v1.12.9）は R3 虚偽UI文言の修正、@154（v1.12.8）で S1 Stage4＝Read切替トグル `CASE_KEY_READ_VIA_MAP` 公開・Backfill 実行済（既存136案件に case_id 付与・診断クリア）
